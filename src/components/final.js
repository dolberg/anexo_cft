...
const Final = ({nextStep, prevStep, values, climate, mo, humedadSuelo, pHSuelo, cambiosProductivos, manejoResiduos, tipoArboles, metodoRiego, fuenteRiego, energiaRiego}) => {
	const Previous = e => {
  		e.preventDefault();
  		prevStep();
	}
	...
	var arbol='';
	var metodoIrrigacion='';
	var fuenteIrrigacion='';
	var energiaIrrigacion='';
	...
	for (var j=0; j< metodoRiego.length; j++){
		if (metodoRiego[j].num == values.tipoRiego){
			metodoIrrigacion=metodoRiego[j].tipo
		}
	}

	for (var k=0; k< fuenteRiego.length; k++){
		if (fuenteRiego[k].num == values.fuenteAgua){
			fuenteIrrigacion=fuenteRiego[k].tipo
		}
	}

	for (var l=0; l< energiaRiego.length; l++){
		if (energiaRiego[l].num == values.fuenteEnergiaRiego){
			energiaIrrigacion=energiaRiego[l].tipo
		}
	}
	...
	return (
    	...
    	<h4> Cambio en el uso de Cultivos de Coberturas</h4>
    	<table>
    		<thead>
					<th></th>
					<th>Tiempo Transcurrido</th>
    	...
    	<Divider variant="middle" />
			...
			<table>
    		<thead>
					<th>Metodo de Irrigación</th>
					<th>Fuente de Agua</th>
				</thead>
				<tbody>
    			<td>{metodoIrrigacion}</td>
    		...
    		<thead>
    			...
    			<td>{values.profundidadIrrigacion} m</td>
    		</tbody>
    	</table>

			<table>
    		<thead>
					<th>Distancia horizontal de la irrigacion en el cultivo</th>
					<th>Litros de agua totales utilizados</th>
				</thead>
				<tbody>
    			<td>{values.distanciaRiego} m</td>
    			<td>{values.litrosRiego} l</td>
    		</tbody>
    	</table>
